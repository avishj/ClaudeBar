...
public struct CopilotUsageProbe: UsageProbe {
    ...
    public func isAvailable() async -> Bool {
        guard let token = credentialStore.get(forKey: CredentialKey.githubToken),
            ...
            Logger.probes.debug("Copilot: Not available - missing token or username")
            return false
        }
        return true
    }
    ...
    public func probe() async throws -> UsageSnapshot {
        guard let token = credentialStore.get(forKey: CredentialKey.githubToken), !token.isEmpty else {
            Logger.probes.error("Copilot: No GitHub token configured")
            throw ProbeError.authenticationRequired
        }

        guard let username = credentialStore.get(forKey: CredentialKey.githubUsername), !username.isEmpty else {
            Logger.probes.error("Copilot: No GitHub username configured")
            throw ProbeError.executionFailed("GitHub username not configured")
        }

        Logger.probes.debug("Copilot: Fetching billing usage for \(username)")
    ...
    private func fetchBillingUsage(username: String, token: String) async throws -> PremiumRequestUsageResponse {
        ...
        Logger.probes.debug("Copilot API response status: \(httpResponse.statusCode)")

        switch httpResponse.statusCode {
        case 200:
            break
        case 401:
            Logger.probes.error("Copilot: Authentication failed (401)")
            throw ProbeError.authenticationRequired
        case 403:
            Logger.probes.error("Copilot: Forbidden - check token permissions (403)")
            throw ProbeError.executionFailed("Forbidden - ensure PAT has 'Plan: read' permission")
        case 404:
            Logger.probes.error("Copilot: User not found or no billing access (404)")
            throw ProbeError.executionFailed("User not found or no billing access")
        default:
            Logger.probes.error("Copilot: HTTP error \(httpResponse.statusCode)")
            throw ProbeError.executionFailed("HTTP error: \(httpResponse.statusCode)")
        }

        // Log raw response for debugging
        if let rawString = String(data: data, encoding: .utf8) {
            Logger.probes.debug("Copilot raw response: \(rawString.prefix(1000))")
        }

        do {
            return try JSONDecoder().decode(PremiumRequestUsageResponse.self, from: data)
        } catch {
            Logger.probes.error("Copilot: Failed to parse response - \(error.localizedDescription)")
    ...
    private func parseUsageResponse(_ response: PremiumRequestUsageResponse, username: String) throws -> UsageSnapshot {
        ...
        Logger.probes.debug("Copilot: Found \(copilotItems.count) Copilot items for \(response.timePeriod.month)/\(response.timePeriod.year)")

        // Log model breakdown
        let modelBreakdown = Dictionary(grouping: copilotItems) { $0.model ?? "Unknown" }
            .mapValues { items in items.reduce(0) { $0 + ($1.grossQuantity ?? 0) } }
        Logger.probes.debug("Copilot models: \(modelBreakdown)")
        ...
        Logger.probes.debug("Copilot: gross=\(Int(totalGrossQuantity)), discount=\(Int(totalDiscountQuantity)), net=\(Int(totalNetQuantity)), amount=\(totalNetAmount)")
        ...
        Logger.probes.debug("Copilot: Used \(Int(used))/\(Int(monthlyLimit)) this month, \(Int(percentRemaining))% remaining")
